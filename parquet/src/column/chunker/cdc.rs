...
impl ContentDefinedChunker {
    ...
    fn calculate<F>(
        ...
        value_selection: ValueSelection<'_>,
        num_levels: usize,
        mut roll_value: F,
    ) -> Vec<CdcChunk>
    where
        F: FnMut(&mut Self, usize),
    {
        ...
        if !has_rep_levels && !has_def_levels {
            ...
            for offset in 0..num_levels {
                let array_index = value_selection.index_at(offset);
            ...
            for offset in 0..num_levels {
                ...
                if def_level == self.max_def_level {
                    let array_index = value_selection.index_at(value_offset);
                    roll_value(self, array_index);
                }
    ...
    fn test_cdc_array_offsets_direct() {
        ...
        let chunks = chunker
            .get_arrow_chunks(
                LevelDataRef::Absent,
                LevelDataRef::Absent,
                ValueSelection::Dense { offset: 0, len: n },
                &array,
            )
            .unwrap();
