...
impl FixedLenByteArraySink<'_> {
    ...
    pub(crate) fn push_selected<'a>(&mut self, values: impl ValueProducer<&'a [u8]>) -> Result<()> {
        ...
        // only — and feed it through a batch instead. Interleaving the block
        // updates with the rest of the per-value work serialises their cache
        // misses; see [`Sbbf::batch`].
...
impl FixedLenByteArraySource for &[FixedLenByteArray] {
    ...
    fn write_to(
        ...
        if let Some(expected) = expected_width {
            if let Some(value) = self.iter().find(|value| value.data().len() != expected) {
