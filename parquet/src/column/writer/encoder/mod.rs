...
use crate::column::writer::{compare_greater_byte_array, is_f16_nan, replace_zero};
...
impl<T: DataType> ColumnChunkEncoder for TypedColumnChunkEncoder<T> {
    ...
    fn flush_data_page(&mut self) -> Result<DataPageValues<T::T>> {
        ...
        let min_value = self
            .min_value
            .take()
            .map(|value| replace_zero(&value, &self.descr, -0.0));
        let max_value = self
            .max_value
            .take()
            .map(|value| replace_zero(&value, &self.descr, 0.0));
...
mod tests {
    ...
    fn packed_fixed_len_byte_array_batches_merge_statistics() {
        ...
        for (batches, min, max) in [
            (all_nan, None, None),
            (mixed, Some([0x00, 0xc0]), Some([0x00, 0x42])),
        ] {
            encoder
            ...
            assert_eq!(
                page.min_value,
                min.map(|value| FixedLenByteArray::from(value.to_vec()))
            );
            assert_eq!(
                page.max_value,
                max.map(|value| FixedLenByteArray::from(value.to_vec()))
            );
        }
