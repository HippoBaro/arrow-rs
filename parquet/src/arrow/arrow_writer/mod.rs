...
pub fn get_column_writers(
    ...
    let plan =
        ArrowWriteSchemaPlan::try_new(parquet, arrow).map_err(|error| error.to_parquet_error())?;
    ArrowColumnWriterFactory::new().create_column_writers(&plan.leaves, props)
}
...
mod tests {
    ...
    fn arrow_writer_large_list_view() {
        ...
        one_column_roundtrip_with_schema(a.clone(), batch.schema());
    ...
    fn test_list_of_struct_roundtrip() {
        ...
        one_column_roundtrip(array.clone(), true);
    ...
    fn arrow_writer_low_cardinality_dictionary_with_bloom_filter() {
        ...
        let mut options = RoundTripOptions::new(Arc::new(dict), true);
        options.bloom_filter = true;
        one_column_roundtrip_with_options(options);
    }
