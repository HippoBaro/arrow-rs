...
use arrow_array::{Array, ArrayRef, RecordBatch, RecordBatchWriter, RunArray};
use arrow_schema::{
...
use crate::column::value::{
    DictionaryKeys, IndexSpan, PhysicalIndexPlan, RunEnds, Sink, ValueSelectionRef,
    gather_run_groups_tiled, gather_tiled, map_values,
};
use crate::column::writer::encoder::{
    ColumnValueEncoder, ColumnValueEncoderImpl, FLBA_BATCH_VALUES, FLBA_COMPUTED_MAX_WIDTH,
    FlbaBatch, FlbaTilePacker,
};
...
use crate::encodings::encoding::{PackedBoolValues, emit_physical_cast, emit_physical_identity};
...
use levels::cursor::{CursorLeafPlan, collect_cursor_leaves, needs_cursor};
use levels::{LeafBatch, LeafBatchSlice, LeafPlan, calculate_array_levels};
...
/// A logical null reachable at a non-nullable `field` returns an error.
pub fn compute_leaves(field: &Field, array: &ArrayRef) -> Result<Vec<ArrowLeafColumn>> {
    if needs_cursor(array.data_type()) {
        return Ok(collect_cursor_leaves(array, field)?
            .into_iter()
            .map(|plan| ArrowLeafColumn(ArrowLeafPlan::Cursor(plan)))
            .collect());
    }

    Ok(calculate_array_levels(array, field)?
        .into_iter()
        .map(|plan| ArrowLeafColumn(ArrowLeafPlan::Direct(plan)))
        .collect())
}
...
impl ArrowColumnWriter {
    ...
    pub fn write(&mut self, col: &ArrowLeafColumn) -> Result<()> {
        match &col.0 {
            ArrowLeafPlan::Direct(plan) => self.write_direct(plan),
            ArrowLeafPlan::Cursor(plan) => self.write_cursor(plan),
        }
    }

    fn write_direct(&mut self, plan: &LeafPlan) -> Result<()> {
        write_leaf(&mut self.writer, plan.view())?;
        Ok(())
    }

    fn write_cursor(&mut self, plan: &CursorLeafPlan) -> Result<()> {
        let mut cursor = plan.cursor(1024, self.cursor_row_limit);
        while let Some(tile) = cursor.next_tile()? {
            write_leaf(&mut self.writer, tile.batch(plan))?;
        }
        Ok(())
    }
    ...
    fn write_with_framer(&mut self, col: &ArrowLeafColumn, framer: &mut CdcFramer) -> Result<()> {
        match &col.0 {
            ArrowLeafPlan::Direct(plan) => self.write_direct_with_framer(plan, framer),
            ArrowLeafPlan::Cursor(plan) => self.write_cursor_with_framer(plan, framer),
        }
    }

    fn write_direct_with_framer(&mut self, plan: &LeafPlan, framer: &mut CdcFramer) -> Result<()> {
        self.write_framed_batch(plan.view(), framer)
    }

    fn write_cursor_with_framer(
        &mut self,
        plan: &CursorLeafPlan,
        framer: &mut CdcFramer,
    ) -> Result<()> {
        let mut cursor = plan.cursor(1024, self.cursor_row_limit);
        while let Some(tile) = cursor.next_tile()? {
            self.write_framed_batch(tile.batch(plan), framer)?;
        }
        Ok(())
    }

    fn write_framed_batch(&mut self, batch: LeafBatch<'_>, framer: &mut CdcFramer) -> Result<()> {
        let frames = framer.frame_arrow_segment(
        ...
        for frame in frames {
            if frame.break_before {
                self.writer.flush_data_page()?;
            }
            let frame = batch.slice(LeafBatchSlice {
                level_offset: frame.level_offset,
                num_levels: frame.num_levels,
                value_offset: frame.value_offset,
                num_values: frame.num_values,
            });
            write_leaf(&mut self.writer, frame)?;
        }
        Ok(())
    }
...
struct ArrowWriteSchemaPlan {
    ...
    /// A descriptor and whether its normalized Arrow schema is `Dictionary<FixedSizeBinary>`.
    /// That layout always uses the byte-array writer so its page representation
    /// remains aligned with the Arrow dictionary reader.
    leaves: Box<[(ColumnDescPtr, bool)]>,
}
...
impl ArrowWriteSchemaPlan {
    fn try_new(
        ...
        let mut leaf_kinds = Vec::with_capacity(parquet_leaves.len());
        let mut fields = Vec::with_capacity(arrow.fields.len());

        for field in &arrow.fields {
            let start = leaf_kinds.len();
            Self::append_leaf_kinds(field.data_type(), &mut leaf_kinds)?;
            fields.push(ArrowTopLevelWriterSpec {
                field: Arc::clone(field),
                leaf_range: start..leaf_kinds.len(),
            });
        }

        if leaf_kinds.len() != parquet_leaves.len() {
        ...
        Ok(Self {
            ...
            leaves: parquet_leaves.iter().cloned().zip(leaf_kinds).collect(),
        })
    }

    fn append_leaf_kinds(
        data_type: &ArrowDataType,
        out: &mut Vec<bool>,
    ) -> std::result::Result<(), ArrowWriteSchemaPlanError> {
        match data_type {
            _ if levels::is_leaf(data_type) => out.push(false),
            ArrowDataType::List(field)
            | ArrowDataType::LargeList(field)
            | ArrowDataType::FixedSizeList(field, _)
            | ArrowDataType::ListView(field)
            | ArrowDataType::LargeListView(field) => {
                Self::append_leaf_kinds(field.data_type(), out)?
            }
            ...
            ArrowDataType::Map(field, _) => match field.data_type() {
                ArrowDataType::Struct(fields) if fields.len() == 2 => {
                    Self::append_leaf_kinds(fields[0].data_type(), out)?;
                    Self::append_leaf_kinds(fields[1].data_type(), out)?;
                }
                _ => {
                    return Err(ArrowWriteSchemaPlanError::General(format!(
                        "Invalid Arrow map type {data_type}: entries must be a two-field struct"
                    )));
                }
            },
            ArrowDataType::Dictionary(_, values) => {
                let mut values = values.as_ref();
                while let ArrowDataType::RunEndEncoded(_, value_field) = values {
                    values = value_field.data_type();
                }
                if matches!(values, ArrowDataType::FixedSizeBinary(_)) {
                    out.push(true);
                } else {
                    Self::append_leaf_kinds(values, out)?;
                }
            }
            ArrowDataType::RunEndEncoded(_, value_field) => {
                Self::append_leaf_kinds(value_field.data_type(), out)?
            }
            _ => {
                return Err(ArrowWriteSchemaPlanError::Nyi(format!(
...
impl ArrowRowGroupWriter {
    ...
    fn validate_batch_shape(&self, batch: &RecordBatch, framer_count: Option<usize>) -> Result<()> {
        ...
        if let Some(actual) = framer_count {
            if actual != self.schema_plan.leaves.len() {
                return Err(ParquetError::General(format!(
...
pub fn get_column_writers(
    ...
    let schema_plan =
        ArrowWriteSchemaPlan::try_new(parquet, arrow).map_err(|e| e.to_parquet_error())?;
    ArrowColumnWriterFactory::new().create_column_writers(&schema_plan.leaves, props)
}
...
impl ArrowColumnWriterFactory {
    ...
    fn create_column_writers(
        &self,
        specs: &[(ColumnDescPtr, bool)],
        props: &WriterPropertiesPtr,
    ) -> Result<Vec<ArrowColumnWriter>> {
        let mut out = Vec::with_capacity(specs.len());
        for (column_index, spec) in specs.iter().enumerate() {
            out.push(self.create_column_writer(spec, props, column_index)?);
        }
        Ok(out)
    }
    ...
    fn create_column_writer(
        &self,
        (descriptor, dictionary_fsb): &(ColumnDescPtr, bool),
        props: &WriterPropertiesPtr,
        column_index: usize,
        ...
        let writer = if *dictionary_fsb {
            ColumnWriter::ByteArrayColumnWriter(GenericColumnWriter::new(
                Arc::clone(descriptor),
                Arc::clone(props),
                page_writer,
            ))
        } else {
            get_column_writer(Arc::clone(descriptor), Arc::clone(props), page_writer)
        };
...
trait ArrowPhysicalBridge<'a>: Copy {
    type Encoder: ColumnValueEncoder;
...
/// Keeping the comparatively large composed plan here makes the Copy
    ...
    B: ArrowPhysicalBridge<'a>,
{
    fn bind(column: &'a dyn arrow_array::Array, selection: ValueSelectionRef<'a>) -> Result<Self> {
        let (values, plan) = dispatch_physical_input(column, selection);
        Ok(Self {
    ...
    B: ArrowPhysicalBridge<'a>,
{
    ...
    #[inline]
    fn plan(self) -> PhysicalIndexPlan<'a> {
        if self.offset == 0 && self.len == self.binding.plan.len() {
            self.binding.plan
        } else {
            self.binding.plan.slice(self.offset, self.len)
        }
    }
}

impl<'binding, 'a, B> ColumnValueSource<B::Encoder> for ArrowPhysicalSource<'binding, 'a, B>
where
    B: ArrowPhysicalBridge<'a>,
{
    #[inline]
    ...
    #[inline]
    fn write_to(self, encoder: &mut B::Encoder) -> Result<()> {
        self.binding.storage.write_values(encoder, self.plan())
    }
    ...
    fn count_variable_width_within_byte_budget(
        ...
        self.binding
            .storage
            .count_variable_width_within_byte_budget(self.plan(), budget, target)
    }
}

fn write_arrow_physical<'a, B>(
    writer: &mut GenericColumnWriter<B::Encoder>,
...
/// Arrow family binds through the same physical-source seam. `Int96` is
/// unreachable — `arrow_to_parquet_type` never emits it.
macro_rules! dispatch_leaf_writer {
    ($writer:expr, $phys:ident) => {
        match $writer {
            ...
            ColumnWriter::FloatColumnWriter(typed) => $phys!(&[f32], typed),
            ColumnWriter::DoubleColumnWriter(typed) => $phys!(&[f64], typed),
...
) -> (&'a dyn arrow_array::Array, PhysicalIndexPlan<'a>) {
    if let Some(dictionary) = column.as_any_dictionary_opt() {
        let keys = dictionary_keys(dictionary.keys());
        (
            dictionary.values().as_ref(),
            PhysicalIndexPlan::dictionary(selection, keys),
        )
    } else {
        (column, PhysicalIndexPlan::identity(selection))
    }
}
...
impl<'a> ArrowPhysicalBridge<'a> for Int32Storage<'a> {
    type Encoder = ColumnValueEncoderImpl<ParquetInt32Type>;
    ...
    fn write_values(self, encoder: &mut Self::Encoder, plan: PhysicalIndexPlan<'a>) -> Result<()> {
        match self {
            Self::Identity(values) => emit_physical_identity(encoder, values, plan),
            Self::Int8(values) => emit_physical_cast(encoder, values, plan, |v| v as i32),
            Self::Int16(values) => emit_physical_cast(encoder, values, plan, |v| v as i32),
            Self::UInt8(values) => emit_physical_cast(encoder, values, plan, |v| v as i32),
            Self::UInt16(values) => emit_physical_cast(encoder, values, plan, |v| v as i32),
            Self::Date64(values) => {
                // Arrow Date64 stores milliseconds; Parquet DATE stores days,
                // so truncate to whole days.
                emit_physical_cast(encoder, values, plan, |v| (v / 86_400_000) as i32)
            }
            // Schema conversion selects INT32 only for decimal precision <= 9,
            // so valid higher-width decimal values narrow losslessly.
            Self::Int64(values) => emit_physical_cast(encoder, values, plan, |v| v as i32),
            Self::Int128(values) => emit_physical_cast(encoder, values, plan, |v| v as i32),
            Self::Int256(values) => {
                emit_physical_cast(encoder, values, plan, |v| v.as_i128() as i32)
            }
        }
    }
}
...
impl<'a> ArrowPhysicalBridge<'a> for Int64Storage<'a> {
    type Encoder = ColumnValueEncoderImpl<ParquetInt64Type>;
    ...
    fn write_values(self, encoder: &mut Self::Encoder, plan: PhysicalIndexPlan<'a>) -> Result<()> {
        match self {
            ...
            // so valid higher-width decimal values narrow losslessly.
            Self::Int128(values) => emit_physical_cast(encoder, values, plan, |v| v as i64),
            Self::Int256(values) => {
                emit_physical_cast(encoder, values, plan, |v| v.as_i128() as i64)
            }
        }
    }
}
...
#[derive(Clone, Copy)]
struct BoolStorage<'a> {
    bytes: &'a [u8],
    bit_offset: usize,
}

impl<'a> ArrowPhysicalBridge<'a> for BoolStorage<'a> {
    type Encoder = ColumnValueEncoderImpl<BoolType>;
    ...
    fn write_values(self, encoder: &mut Self::Encoder, plan: PhysicalIndexPlan<'a>) -> Result<()> {
        encoder.write_packed_bool(PackedBoolValues::new_physical(
            self.bytes,
            self.bit_offset,
            plan,
        ))
...
/// Already-downcast storage for Arrow types represented by Parquet FLBA.
#[derive(Clone, Copy)]
enum FixedLenByteArrayStorage<'a> {
...
impl<'a> ArrowPhysicalBridge<'a> for FixedLenByteArrayStorage<'a> {
    type Encoder = ColumnValueEncoderImpl<FixedLenByteArrayType>;
    ...
    fn write_values(self, encoder: &mut Self::Encoder, plan: PhysicalIndexPlan<'a>) -> Result<()> {
        if plan.is_grouped() {
            encoder.begin_flba_run(plan.len());
        } else {
        ...
        match self {
            Self::Fixed(array) => {
                let values = map_values(plan, move |i| array.value(i));
                if plan.is_grouped() {
                    gather_run_groups_tiled::<FLBA_BATCH_VALUES, _, _>(values, |values, counts| {
                        encoder.commit(FlbaBatch::RunGroups { values, counts })
                    })
                } else {
                    gather_tiled::<FLBA_BATCH_VALUES, _, _, _>(values, |values| {
                        encoder.flush_gathered(values)
                    })
                }
            }
            // Parquet INTERVAL is 12 little-endian bytes: four each for months,
            // days, and milliseconds. YearMonth fills only months; DayTime fills
            // days and milliseconds, leaving the unused fields zeroed.
            Self::YearMonth(values) => {
                write_computed_fixed_len_values(plan, encoder, 12, move |i, dest| {
                    dest[..4].copy_from_slice(&values[i].to_le_bytes());
                    dest[4..].fill(0);
                })
            }
            Self::DayTime(values) => {
            ...
            Self::Decimal32 { values, width } => {
                write_computed_fixed_len_values(plan, encoder, width, move |i, dest| {
                    dest.copy_from_slice(&values[i].to_be_bytes()[(4 - width)..])
                })
            }
            Self::Decimal64 { values, width } => {
                write_computed_fixed_len_values(plan, encoder, width, move |i, dest| {
                    dest.copy_from_slice(&values[i].to_be_bytes()[(8 - width)..])
                })
            }
            Self::Decimal128 { values, width } => {
                write_computed_fixed_len_values(plan, encoder, width, move |i, dest| {
                    dest.copy_from_slice(&values[i].to_be_bytes()[(16 - width)..])
                })
            }
            Self::Decimal256 { values, width } => {
                write_computed_fixed_len_values(plan, encoder, width, move |i, dest| {
                    dest.copy_from_slice(&values[i].to_be_bytes()[(32 - width)..])
                })
            }
            Self::Float16(values) => {
                write_computed_fixed_len_values(plan, encoder, 2, move |i, dest| {
                    dest.copy_from_slice(&values[i].to_le_bytes())
                })
            }
        }?;
        encoder.finish_flba();
...
fn write_computed_fixed_len_values<F>(
    plan: PhysicalIndexPlan<'_>,
    sink: &mut ColumnValueEncoderImpl<FixedLenByteArrayType>,
    width: usize,
    write_at: F,
) -> Result<()>
where
...
{
    if plan.is_grouped() {
        let mut buf = [0u8; FLBA_COMPUTED_MAX_WIDTH];
        return plan.try_for_each_index_group(|index, count| {
            write_at(index, &mut buf[..width]);
            sink.commit(FlbaBatch::RunGroups {
                values: &[&buf[..width]],
                counts: &[count],
            })
        });
    ...
    let mut packer = FlbaTilePacker::new(sink, width);
    plan.try_for_each_index(|index| packer.push(|dest| write_at(index, dest)))?;
    packer.finish()
}

impl<'a> ArrowPhysicalBridge<'a> for &'a [f32] {
    type Encoder = ColumnValueEncoderImpl<ParquetFloatType>;
    ...
    fn write_values(self, encoder: &mut Self::Encoder, plan: PhysicalIndexPlan<'a>) -> Result<()> {
        emit_physical_identity(encoder, self, plan)
    }
}

impl<'a> ArrowPhysicalBridge<'a> for &'a [f64] {
    type Encoder = ColumnValueEncoderImpl<ParquetDoubleType>;
...
mod tests {
    ...
    fn arrow_write_schema_plan_caches_nested_leaf_ranges() {
        ...
        // The deprecated one-shot entry point uses the same recipe path.
        let props = Arc::new(WriterProperties::default());
        #[allow(deprecated)]
    ...
    fn arrow_row_group_factory_rejects_parquet_leaf_count_mismatch_cleanly() {
        ...
        let err = match factory.create_column_writers(0) {
            Ok(_) => panic!("mismatched schemas should not create column writers"),
            Err(err) => err,
        };
        assert!(
            err.to_string()
    ...
    fn arrow_writer_clustered_nullable_ranges_roundtrip_all_families() {
        const ROWS: usize = 80;
        let valid = |idx: usize| (7..39).contains(&idx) || (47..76).contains(&idx);
    ...
    #[test]
    fn arrow_writer_mixed_run_end_and_dense_dictionary_preserves_order() {
    ...
    /// Fixed-len byte-array REE values use the run-buffered dictionary path. A
    /// single long run does not reserve or retain one dictionary index per
    /// logical row before the data page is flushed.
    #[test]
    ...
    fn arrow_writer_run_end_encoded_byte_values() {
        fn run_end_roundtrip(run_ends: Int32Array, values: ArrayRef) -> (ArrayRef, Vec<Encoding>) {
            ...
            assert_ree_file_matches_dense(ree.clone(), None);
            let schema = Arc::new(Schema::new(vec![Field::new(
                "c",
                ree.data_type().clone(),
                true,
    ...
    #[test]
    fn arrow_writer_dictionary_schema_accepts_run_end_byte_batches() {
    ...
    /// Native CDC preserves exact dense boundaries and semantic parity across
    /// nullability, slicing, value families, and every supported run-end width.
    #[test]
    fn arrow_writer_run_end_encoded_cdc_matrix() {
        let props = WriterProperties::builder()
        ...
        let native_plan = compute_leaves(&nullable_field, &ree_nullable).unwrap();
        assert_eq!(native_plan.len(), 1);
        assert!(matches!(native_plan[0].0, ArrowLeafPlan::Cursor(_)));
        ...
        // Exact bytes additionally pin the rolling-hash chunk boundaries.
        ...
        let ree: ArrayRef = Arc::new(Int32RunArray::try_new(&run_ends, &values).unwrap());
        assert_ree_file_matches_dense(ree, Some(props));
    ...
    #[test]
    fn arrow_writer_list_view_of_ree_cdc_matches_reordered_dense_child() {
        ...
        let props = WriterProperties::builder()
            ...
            .build();
        let native_field = Field::new("c", native.data_type().clone(), false);
        let dense_field = Field::new("c", dense.data_type().clone(), false);
        assert_eq!(
            write_column_without_arrow_metadata_as(native_field, native, Some(props.clone())),
            write_column_without_arrow_metadata_as(dense_field, dense, Some(props)),
        );
    }

    #[test]
    fn arrow_writer_run_end_encoded_nested_in_struct_with_content_defined_chunking() {
        // Parent nesting does not determine the REE route. This REE still has a
        // leaf value type, so it remains on the native leaf route under CDC.
        let num_runs = 300usize;
        let run_len = 3usize;
        let run_ends = Int32Array::from_iter_values((1..=num_runs).map(|r| (r * run_len) as i32));
        let values =
    ...
    #[test]
    fn arrow_writer_run_end_encoded_bytes_respect_page_budget() {
    ...
    #[test]
    fn arrow_writer_ree_cursor_tiles_do_not_reset_page_admission() {
        let rows = 20_000;
        let ree: ArrayRef = Arc::new(
    ...
    /// A run-end byte column must survive dictionary -> PLAIN fallback. Two
    /// batches of disjoint run values under a tiny dictionary-page-size limit
    /// force the second batch onto PLAIN. The first batch's run-buffered indices
    ...
    /// the REE path interns the complete run-value batch ahead of the fallback
    /// check. REE targets low cardinality, so high-cardinality REE can exceed
    /// the dict-page-size limit within one batch.
    #[test]
    ...
    #[test]
    fn arrow_writer_run_end_encoded_nullable_parent_sparse_gap() {
    ...
    /// Dense logical equivalent of a run-end-encoded array (`take`-expanded),
    /// used as the oracle for route-parity tests below.
    ...
    fn arrow_writer_dense_batches_under_nested_wrapper_schemas() {
        ...
        let assert_direct = |field: Field, actual: ArrayRef, expected: &ArrayRef| {
            assert!(
                compute_leaves(&field, &actual)
                    .unwrap()
                    .iter()
                    .all(|leaf| matches!(&leaf.0, ArrowLeafPlan::Direct(_)))
            );
            assert_eq!(
        ...
        // A dense list under an REE<List> schema must retain the direct route;
        ...
        assert_direct(nested_ree, dense.clone(), &dense);
        ...
        assert_direct(dictionary_ree, dense.clone(), &dense);
        ...
        assert_direct(ree_dictionary, dense.clone(), &dense);
        ...
        assert_direct(wrapped_struct, dense_struct.clone(), &dense_struct);
        ...
        assert_direct(wrapped_list, list.clone(), &list);
        ...
        assert_direct(list_of_dictionary, list.clone(), &list);
        assert_direct(
        ...
        assert_direct(wrapped_map, map.clone(), &map);
    }
    ...
    /// schema.
    #[test]
    fn arrow_writer_run_end_encoded_nonleaf_merges_nullability() {
    ...
    /// An REE write must read back identically to writing its dense
    /// (`take`-expanded) equivalent. Depending on the input and writer
    /// properties, the REE may use a native route or be expanded by planning.
    ...
    /// Assert semantic parity for an REE input and its dense logical
    /// equivalent. Native REE transport may choose different ordinary page
    /// and dictionary boundaries.
    fn assert_ree_file_matches_dense(ree: ArrayRef, props: Option<WriterProperties>) {
        let dense = ree_dense_equivalent(&ree);
        assert_eq!(
    ...
    /// Build the same run/key shape over an arbitrary dictionary value family.
    /// Both a null key and a key that addresses a null dictionary value are
    /// present, and every run is long enough to exercise the native CDC route.
    ...
    #[test]
    fn arrow_writer_run_end_encoded_dictionary_composition_leaf_matrix() {
        ...
        for (family, values) in nullable_physical_values() {
            ...
            assert!(matches!(leaves[0].0, ArrowLeafPlan::Cursor(_)), "{family}");

            for target in [ree.clone(), ree.slice(29, 301)] {
                assert_ree_file_matches_dense(target.clone(), Some(page_props.clone()));
                assert_ree_file_matches_dense(target.clone(), Some(tiny_page_props.clone()));
                assert_ree_file_matches_dense(target, Some(cdc_props.clone()));
            }
        }
    }
    ...
    fn arrow_writer_flba_physical_plan_preserves_ranges_repeats_and_cursor_roundtrip() {
        ...
        let (physical, plan) = dispatch_physical_input(
            dictionary.as_ref(),
        ...
        let run_dictionary: ArrayRef =
            Arc::new(DictionaryArray::<Int32Type>::try_new(run_keys, values).unwrap());
    ...
    #[test]
    fn bound_physical_sources_slice_the_plan_without_retaining_arrays() {
    ...
    #[test]
    fn arrow_writer_run_end_encoded_dictionary_composition_memory_is_run_bounded() {
    ...
    /// Rep-free dictionary fields inside run-encoded structs exercise the same
    /// mapping composition independently for every physical family. The plan
    /// may resolve keys at physical-run cardinality, but must preserve the run
    /// wrapper and must never build a logical-row-sized child.
    #[test]
    fn arrow_writer_ree_struct_dictionary_composition_matrix() {
        let keys = Int32Array::from(vec![Some(0), Some(1), None, Some(2), Some(0), Some(1)]);
        ...
        assert!(
            leaves
                .iter()
                .all(|leaf| matches!(leaf.0, ArrowLeafPlan::Cursor(_)))
        );
        ...
        let props = WriterProperties::builder()
            ...
            .build();
        assert_ree_file_matches_dense(ree.clone(), Some(props.clone()));
        assert_ree_file_matches_dense(ree.slice(17, 151), Some(props));
    }

    /// Dictionaries below a repeated run-value node resolve into compact
    /// terminal-index tiles. This combines struct and list nesting with
    /// numeric, boolean, byte-array, and FLBA dictionaries, including null
    /// lists, null structs, null keys, null dictionary values, slicing, and
    /// page boundaries.
    #[test]
    fn arrow_writer_ree_list_struct_dictionary_composition_matrix() {
        ...
        for target in [ree.clone(), ree.slice(1, 12)] {
            ...
            let native = roundtrip_column_without_arrow_metadata(target, Some(props.clone()));
            let expected = roundtrip_column_without_arrow_metadata(dense, Some(props.clone()));
            assert_eq!(native.as_ref(), expected.as_ref());
        }
    }

    /// A repeated parent adds an outer list-position mapping in front of the
    /// run -> key -> value chain. Numeric and byte dictionaries take different
    /// planning routes and produce the same file as dense input for full and
    /// sliced list windows.
    #[test]
    fn arrow_writer_list_of_ree_dictionary_composition_parity() {
        let props = WriterProperties::builder()
    ...
    #[test]
    fn arrow_writer_ree_nested_dictionary_values() {
    ...
    /// Run-end-encoded struct-of-leaves is lowered natively without dense
    /// materialization. Nulls at every level and page boundaries split runs.
    #[test]
    fn arrow_writer_ree_struct_roundtrip() {
        let ree = ree_struct_test_array();
        ...
        let native = roundtrip_column(Field::new("c", ree.data_type().clone(), true), ree, None);
        let a = native.as_struct().column(0).as_primitive::<Int32Type>();
        let mut expected: Vec<Option<i32>> = Vec::new();
    ...
    /// Repeated terminal groups feed `RLE_DICTIONARY` index runs directly.
    #[test]
    fn arrow_writer_ree_struct_uses_rle_dictionary() {
        let ree = ree_struct_test_array();
        let schema = Arc::new(Schema::new(vec![Field::new(
            "c",
            ree.data_type().clone(),
        ...
        // Leaves 0 (int32) and 1 (utf8) adopt the run structure into
        // RLE_DICTIONARY index runs; leaf 2 (bool) has no dictionary encoding.
        for i in 0..2 {
            assert!(
                rg.column(i)
                    .encodings()
    ...
    #[test]
    fn arrow_writer_ree_struct_under_nullable_struct() {
        let ree = ree_struct_test_array();
        ...
        let (native_field, native_col) = make_outer(ree.clone());
        let (dense_field, dense_col) = make_outer(ree_dense_equivalent(&ree));

        let plans = compute_leaves(&native_field, &native_col).unwrap();
        ...
        for props in [
            ...
            let native = roundtrip_column(native_field.clone(), native_col.clone(), props.clone());
            let dense = roundtrip_column(dense_field.clone(), dense_col.clone(), props);
            assert_eq!(native.as_ref(), dense.as_ref());
        }
    }

    /// Nested structs (struct-of-struct) and an FLBA leaf (decimal128) under REE.
    #[test]
    fn arrow_writer_ree_struct_nested() {
    ...
    #[test]
    fn arrow_writer_ree_struct_no_nulls() {
    ...
    /// A run-end struct carrying a dictionary field resolves keys at run
    /// granularity instead of hydrating the whole struct densely. Covers null run values,
    /// null keys, page splits, and a sliced (mid-run) window.
    #[test]
    fn arrow_writer_ree_struct_with_dictionary_field() {
        ...
        // Sliced window (starts and ends mid-run): the decode's values-child
        // swap must preserve the logical length/offset.
    ...
    #[test]
    fn arrow_writer_list_of_ree_struct_with_dictionary_field() {
        ...
        let ree = Int32RunArray::try_new(&run_ends, &values).unwrap();
        // Four non-empty list rows over the 10 logical rows keep this fixture
        // on the fused route.
    ...
    #[test]
    fn arrow_writer_ree_struct_mixed_dictionary_positions() {
        // a: rep-free dictionary — decoded to a flat leaf.
    ...
    /// Multiple batches into one writer: each batch contributes its own leaf
    /// plan to the same column chunk (with a dictionary shared across plans).
    #[test]
    fn arrow_writer_ree_struct_multi_batch() {
        let ree = ree_struct_test_array();
        let schema = Arc::new(Schema::new(vec![Field::new(
            "c",
            ree.data_type().clone(),
        ...
        let batches: Vec<RecordBatch> = reader.map(|b| b.unwrap()).collect();
        let arrays: Vec<&dyn Array> = batches.iter().map(|b| b.column(0).as_ref()).collect();
        let native = arrow_select::concat::concat(&arrays).unwrap();
    ...
    #[test]
    fn arrow_writer_ree_list_roundtrip() {
    ...
    /// Grouped byte-array dictionary writes preserve the page's exact
    /// unencoded-byte total without re-interning every logical occurrence.
    #[test]
    fn arrow_writer_ree_list_strings_unencoded_bytes() {
    ...
    /// dictionary values.
    #[test]
    fn arrow_writer_ree_list_dict_roundtrip() {
    ...
    #[test]
    fn arrow_writer_ree_struct_with_list() {
    ...
    #[test]
    fn arrow_writer_ree_list_under_nullable_struct() {
        let ree = ree_list_test_array();
    ...
    /// A boolean-valued dictionary leaf under a repeated list uses the same
    /// terminal-index path as every other dictionary leaf.
    #[test]
    fn arrow_writer_ree_list_of_bool_dictionary() {
        // The arrow reader cannot reconstruct `Dictionary<Boolean>` (its
        // dictionary packing covers numeric value types only, independent of
        // the write path), so read both files
        // without the embedded arrow schema: each reconstructs as a plain
        // `List<Boolean>` column and the two must agree.
        fn roundtrip_no_hint(field: Field, col: ArrayRef) -> ArrayRef {
            let schema = Arc::new(Schema::new(vec![field]));
            let batch = RecordBatch::try_new(schema.clone(), vec![col]).unwrap();
            let mut file = vec![];
            let mut writer = ArrowWriter::try_new(&mut file, schema, None).unwrap();
        ...
        for target in [ree.clone(), ree.slice(1, 5)] {
            let native = roundtrip_no_hint(
                Field::new("c", target.data_type().clone(), true),
                target.clone(),
            );
            let dense = ree_dense_equivalent(&target);
            let via_dense =
                roundtrip_no_hint(Field::new("c", dense.data_type().clone(), true), dense);
            assert_eq!(native.as_ref(), via_dense.as_ref());
        }
    }
    ...
    /// levels.
    #[test]
    fn arrow_writer_ree_list_nested() {
    ...
    /// Fixed-size-list run values use the bounded cursor path too.
    #[test]
    fn arrow_writer_ree_fixed_size_list() {
    ...
    /// Content-defined chunking consumes the same native non-leaf run-end plan
    /// as the ordinary writer at any nesting depth; results match the dense
    /// oracle.
    #[test]
    fn arrow_writer_ree_nonleaf_with_content_defined_chunking() {
        let props = WriterProperties::builder()
            .set_content_defined_chunking(Some(CdcOptions::default()))
            .build();
        ...
        assert_ree_file_matches_dense(struct_ree, Some(props.clone()));
        assert_ree_file_matches_dense(list_ree, Some(props));
    }

    #[test]
    fn arrow_writer_cdc_ree_is_invariant_to_record_batch_partitioning() {
        let ree = ree_list_test_array();
        let props = WriterProperties::builder()
            .set_content_defined_chunking(Some(CdcOptions {
                min_chunk_size: 1,
    ...
    #[test]
    fn arrow_writer_cdc_ree_is_invariant_across_cursor_tiles() {
        let list = ListArray::new(
    ...
    #[test]
    fn arrow_writer_ree_list_dict_fallback() {
        let n_runs = 64usize;
    ...
    /// Route parity for a list with a run-end child across run boundaries,
    /// null runs, empty and null list rows, and page splits. The fine-grained
    /// row shapes deliberately allow the planner to choose the generic route.
    #[test]
    fn arrow_writer_list_of_ree_roundtrip() {
        ...
        // Spot-check the logical rows explicitly.
        let native = roundtrip_column(Field::new("c", list.data_type().clone(), true), list, None);
        let rows = native.as_list::<i32>();
    ...
    /// Route parity for struct-valued runs under a list, including nullable
    /// scalar and byte-array descendants.
    #[test]
    fn arrow_writer_list_of_ree_struct() {
    ...
    /// Route parity for sliced list windows and for a list-of-runs column under
    /// a nullable outer struct.
    #[test]
    fn arrow_writer_list_of_ree_sliced_and_nested() {
        let list = list_of_ree_test_array();
        ...
        let (native_field, native_col) = make_outer(list.clone());
        let (dense_field, dense_col) = make_outer(list_of_ree_dense_equivalent(&list));
        let native = roundtrip_column(native_field, native_col, None);
        let dense = roundtrip_column(dense_field, dense_col, None);
        assert_eq!(native.as_ref(), dense.as_ref());
    }
    ...
    fn list_of_ree_lists_test_array() -> ArrayRef {
        ...
        // Outer rows: [e0,e1], [], [e2,e3,e4] (opens mid-run → override
        // fragment), NULL, [e5..e9] (crosses three runs).
    ...
    /// Route parity for a list of REE-of-lists with runs crossing outer row
    /// boundaries, null/empty inner lists, null/empty outer rows, and page
    /// splits.
    #[test]
    fn arrow_writer_list_of_ree_lists_roundtrip() {
    ...
    /// Equal-width outer rows wholly inside one child run reuse an exact leaf
    /// record. The first run ends mid-row so the crossing row must fall back to
    /// ordinary traversal; 2,080 leaf slots also cross cursor tile boundaries.
    #[test]
    fn arrow_writer_list_of_ree_lists_record_replay() {
    ...
    /// Route parity for struct-valued runs that combine a rep-free leaf and a
    /// list-bearing leaf under the same outer list.
    #[test]
    fn arrow_writer_list_of_ree_struct_with_list() {
        ...
        // Sliced windows and a nullable outer struct use the same route oracle.
    ...
    /// LargeList takes the fused path too.
    #[test]
    fn arrow_writer_large_list_of_ree() {
    ...
    fn ree_run_group_batch_boundaries_and_bloom_filters() {
        ...
        let bloom = WriterProperties::builder()
            .set_bloom_filter_enabled(true)
            .build();
    ...
    // Float16 run values (FIXED_LEN_BYTE_ARRAY, 2 bytes) exercise the
    // computed-FLBA run-end emit path (FlbaRunEnd::computed) and the
    // descr-aware Float16 min/max stats fold.
    #[test]
    fn ree_float16() {
    ...
    // Float16 REE with NaN, +0.0 and -0.0 run values: the per-run min/max stats
    // fold must normalize signed zero and exclude NaN exactly like the dense
    // path. NaN breaks array equality, so assert the stronger byte-identity —
    // if the native REE bytes equal the dense bytes, the statistics match too.
    #[test]
    fn ree_float16_nan_and_signed_zero() {
        ...
        assert_eq!(
            ...
            "native REE Float16 output (values and NaN/±0 statistics) must match dense"
        );
    }

    // Decimal run values exercise every computed-FLBA physical-width bridge and
    // the descriptor-aware decimal min/max fold.
    #[test]
    fn ree_decimals() {
        ...
        macro_rules! check {
            ($array:ty, $value:ty, $precision:expr, $width:expr) => {{
                let values = <$array>::from(vec![Some(100 as $value), None, Some(-55 as $value)])
                    .with_precision_and_scale($precision, 2)
                    .unwrap();
                let ree: ArrayRef =
    ...
    #[test]
    fn arrow_writer_run_end_encoded_delta_encodings() {
    ...
    // A FixedSizeList whose values child is run-end encoded: REE nested under a
    // fixed-size-list container (rep_level > 0) takes the per-row RunLeaf
    // route. This is the container-over-REE direction, distinct from the inverse
    // `arrow_writer_ree_fixed_size_list` (REE whose values are a FixedSizeList).
    #[test]
    fn arrow_writer_fixed_size_list_of_ree() {
        ...
        for (offset, len) in [(0, 3), (1, 2), (2, 1)] {
            let native_col = fsl.slice(offset, len);
            let dense_col = dense_fsl.slice(offset, len);
            let native = roundtrip_column(
                Field::new("c", native_col.data_type().clone(), true),
                native_col,
                None,
            );
            let via_dense = roundtrip_column(
    ...
    fn arrow_writer_map_of_ree() {
        ...
        let native = roundtrip_column(Field::new("c", map.data_type().clone(), true), map, None);
        let via_dense = roundtrip_column(
    ...
    #[test]
    fn ree_arbitrary_wrapper_nesting() {
