    ...
    for (depth, node) in path.iter().enumerate() {
        ...
        let (kind, child) = match node.kind {
            ...
            NodeKind::RunEndEncoded | NodeKind::Dictionary => {
                unreachable!("indexed node reached direct-path binding")
            }
        };
...
impl<'a> DirectGroupCursor<'a> {
    pub(crate) fn try_new(plans: &'a [&'a CursorLeafPlan], target_rows: usize) -> Option<Self> {
        ...
        let tiles = plans
            .iter()
            .map(|plan| LeafTile::new(plan.max_def_level, plan.max_rep_level, false))
            .collect();
        Some(Self {
            plans,
            next_row: 0,
...
fn collect_node(
    ...
    let result = (|| match kind {
        ...
        NodeKind::Null | NodeKind::Leaf | NodeKind::DictionaryLeaf => {
            let direct_path = (!indexed_traversal)
                .then(|| bind_direct_path(root, root_field, path))
                .transpose()?;
