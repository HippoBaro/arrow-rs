...
/// nesting depth and dictionaries around nested, `Utf8View`, or `BinaryView` values.
fn normalize_metadata_field(field: &Field) -> Field {
    match field.data_type() {
        ...
        DataType::Dictionary(key, value_type) => {
            ...
            // Parquet expands nested dictionary values into leaf columns, and
            // the reader has no dictionary reader for view values.
    ...
    match dt {
        ...
        DataType::Dictionary(_, value) => {
            value.is_nested()
                ...
                || needs_metadata_normalization(value)
        }
        _ => false,
    }
}
