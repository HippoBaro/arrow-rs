...
#[inline]
fn common_prefix_length(previous: &[u8], value: &[u8]) -> usize {
    let prefix_len = std::cmp::min(previous.len(), value.len());
    let mut match_len = 0;
    while match_len < prefix_len && previous[match_len] == value[match_len] {
        match_len += 1;
    }
    match_len
}
